...
function prepareGameField() { // Генерация поля
...
function startGame() { //Запуск игры нажатием на кнопку snake-start
    ...
    setTimeout(createFood, 0); //Без задержек
    setTimeout(createObstacles, 1000); // первый раз препятствие через 1000 мс
    myInterval = setInterval(createObstacles, 10000); // затем каждые 10000 мс
}
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood();
        score++;
        currentScore.innerHTML = 'Текущий счет: ' + score;
    }
    return check;
}
